...
bool SQLiteLoaderService::createTables() {
    ...
    if (!tableExists("branded_foods")) {
        ...
        if (rc != SQLITE_OK) {
            std::cerr << "Error creating branded_foods table: " << errMsg << std::endl;
            sqlite3_free(errMsg);
            return false;
        }
    }
...
bool SQLiteLoaderService::LoadBrandedFood(
    ...
    std::vector<std::string> columns = {
        ...
        "market_country", "preparation_state_code", "trade_channel",
    ...
    for (const auto &food : branded_foods) {
        ...
        food.brand_owner ? sqlite3_bind_text(stmt, idx++, food.brand_owner->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.brand_name ? sqlite3_bind_text(stmt, idx++, food.brand_name->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.subbrand_name ? sqlite3_bind_text(stmt, idx++, food.subbrand_name->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.gtin_upc ? sqlite3_bind_text(stmt, idx++, food.gtin_upc->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.ingredients ? sqlite3_bind_text(stmt, idx++, food.ingredients->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.not_a_significant_source_of ? sqlite3_bind_text(stmt, idx++, food.not_a_significant_source_of->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        ...
        food.serving_size_unit ? sqlite3_bind_text(stmt, idx++, food.serving_size_unit->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.household_serving_fulltext ? sqlite3_bind_text(stmt, idx++, food.household_serving_fulltext->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.branded_food_category ? sqlite3_bind_text(stmt, idx++, food.branded_food_category->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.data_source ? sqlite3_bind_text(stmt, idx++, food.data_source->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.package_weight ? sqlite3_bind_text(stmt, idx++, food.package_weight->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);

        // Handle date fields by converting year_month_day to string
        if (food.modified_date) {
            std::string dateStr = std::to_string(static_cast<int>(food.modified_date->year())) + "-" +
        ...
        if (food.available_date) {
            std::string dateStr = std::to_string(static_cast<int>(food.available_date->year())) + "-" +
        ...
        if (food.discontinued_date) {
            std::string dateStr = std::to_string(static_cast<int>(food.discontinued_date->year())) + "-" +
                                 std::to_string(static_cast<unsigned>(food.discontinued_date->month())) + "-" +
        ...
        food.market_country ? sqlite3_bind_text(stmt, idx++, food.market_country->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.preparation_state_code ? sqlite3_bind_text(stmt, idx++, food.preparation_state_code->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.trade_channel ? sqlite3_bind_text(stmt, idx++, food.trade_channel->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.short_description ? sqlite3_bind_text(stmt, idx++, food.short_description->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
        food.material_code ? sqlite3_bind_text(stmt, idx++, food.material_code->c_str(), -1, SQLITE_TRANSIENT) : sqlite3_bind_null(stmt, idx++);
    ...
    if (success) {
        commitTransaction();
        std::cout << "Successfully loaded " << count << " branded food records" << std::endl;
    } else {
        rollbackTransaction();
        std::cout << "Failed to load branded foods. Rolling back transaction." << std::endl;
    }
